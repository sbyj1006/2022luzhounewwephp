...
class News extends Frontend
{
    ...
    public function index(){
        ...
        if($id){
            ...
            $fistdata=Db::name('category')->where(array('pid'=>3))->order("weigh asc")->find();
            $id=$fistdata['id'];
        }
        $map['status']=1;
        ...
        $newstj=DB::name('article')->where($map)->order('sort desc')->order('addtime desc')->limit(6)->select();

        foreach ($newstj as $key=>$value){
            $newstj[$key]['typename']=Db::name('category')->where(array('id'=>$value['tid']))->value('name');
        }
