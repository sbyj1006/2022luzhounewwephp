...
class Article extends Model
{
    ...
    public function tysh_list($id){//
        $map['tid']=$id;
        $map['status']=1;//已发布
        $list=DB::name('article')->where($map)->order('sort desc')->order('addtime desc')->paginate(9,false,['query' => request()->param()]);
        $page = $list->render();
        $result = array(
            'page' => $page,
            'list'=>$list,
    ...
    public function shzr_list($id){//社会责任
        $map['tid']=$id;
        $map['status']=1;//已发布
        $list=DB::name('article')->where($map)->order('sort desc')->order('addtime desc')->paginate(5,false,['query' => request()->param()]);
        $page = $list->render();
        $result = array(
            'page' => $page,
            'list'=>$list,
    ...
    public function news_list($id){//资讯分页
        $map['tid']=$id;
        $map['status']=1;//已发布
        $list=DB::name('article')->where($map)->order('sort desc')->order('addtime desc')->paginate(8,false,['query' => request()->param()]);
        $page = $list->render();
        $result = array(
            'page' => $page,
            'list'=>$list,
    ...
    public function cases_list($id){//资讯分页
        $map['tid']=$id;
        $map['status']=1;//已发布
        $list=DB::name('article')->where($map)->order('sort desc')->order('addtime desc')->paginate(6,false,['query' => request()->param()]);
        $page = $list->render();
        $result = array(
            'page' => $page,
            'list'=>$list,
